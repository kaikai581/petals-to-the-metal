#!/usr/bin/env python
'''
This script trains one of the simplest WRN models.
'''
...
def wrn10_4_d8d4d4(**kwargs):
    return Wide_ResNet(10, 4, 0.3, initial_stride=1, N=8, f=True, r=2, **kwargs)

if __name__ == '__main__':
    # command line arguments
    parser = argparse.ArgumentParser()
    parser.add_argument('-l', '--load_weights', type=str, default='models/wrn10_4_d8d4d4_lr1.0e-03_epoch2.pt')
    parser.add_argument('-lr', '--learning_rate', type=float, default=1e-3)
    parser.add_argument('-n', '--nepochs', type=int, default=25)
    parser.add_argument('-r', '--resize_pixel', type=int, default=224)
    ...
    e2cnn_model1 = wrn10_4_d8d4d4(num_classes=get_nclasses(data_transforms))
    # print the model size
    print('wrn10_4_d8d4d4\'s trainable parameters:', model_size_trainable(e2cnn_model1))
    e2cnn_model1 = e2cnn_model1.to(device)
        
    # if model is not trained, train it
    # otherwise, evaluate the model
    model_fpn = 'models/{}x{}/wrn10_4_d8d4d4_lr{:1.1e}_{}_epoch{}.pt'.format(img_size, img_size, learning_rate, 'adam' if use_adam else 'sgd', nepochs)
