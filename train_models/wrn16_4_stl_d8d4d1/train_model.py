#!/usr/bin/env python
'''
Make the simplest network from the e2cnn author simpler by reducing the widening factor.
'''
...
def wrn16_4_stl_d8d4d1(**kwargs):
    return Wide_ResNet(16, 4, 0.3, initial_stride=2, N=8, f=True, r=3, **kwargs)

if __name__ == '__main__':
    # command line arguments
    parser = argparse.ArgumentParser()
    parser.add_argument('-l', '--load_weights', type=str, default='models/wrn16_4_stl_d8d4d1_lr1.0e-03_epoch2.pt')
    ...
    e2cnn_model1 = wrn16_4_stl_d8d4d1(num_classes=get_nclasses(data_transforms))
    # print the model size
    print('wrn16_4_stl_d8d4d1\'s trainable parameters:', model_size_trainable(e2cnn_model1))
    e2cnn_model1 = e2cnn_model1.to(device)
        
    # if model is not trained, train it
    # otherwise, evaluate the model
    model_fpn = 'models/{}x{}/wrn16_4_stl_d8d4d1_lr{:1.1e}_{}_epoch{}.pt'.format(img_size, img_size, learning_rate, 'adam' if use_adam else 'sgd', nepochs)
